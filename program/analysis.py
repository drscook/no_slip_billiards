...
def smoother(max_distort=50):
    ...
    """
    global t_hist, pos_hist, vel_hist, rot_hist, spin_hist
    ...
    for (i, ddt) in enumerate(ddts):
        ...
        for f in range(num_frames[i]):
            ...
            w = [r.dot(s) for (r,s) in zip(re_rot[-1],drot)] # rotates each particle the right amount
            re_rot.append(np.array(w))
            re_spin.append(re_spin[-1])

    re_t = np.asarray(re_t)
    re_pos = np.asarray(re_pos)
    re_vel = np.asarray(re_vel)
    re_rot = np.asarray(re_rot)
    ...
    return re_t, re_pos, re_vel, re_rot, re_spin
...
def compute_particle_boundaries():
    ...
    for s in range(S):
        for p in range(P):
            bdy[s,p] = re_rot[s,p].dot(bdy[s,p]) + re_pos[s,p]
    return bdy
